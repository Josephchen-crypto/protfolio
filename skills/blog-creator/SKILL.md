...
**Cover URL must be verified before writing.** When Joseph provides a `cover` URL (GitHub raw, CDN, etc.), always run a HEAD/HEAD-equivalent request (`curl -sI` or `curl -I`) to confirm `HTTP 200` **and** the resource is actually an image (`content-type: image/...`). A 200 on a non-image (HTML error page, 404 redirect to a generic page) breaks the OG card and the blog card image. Non-ASCII characters in the URL path (e.g. `路.jpeg`) will trip a security scan — that's expected for legitimate Chinese filenames, approve and continue. Pitfall: don't trust the URL surface — verify before pasting into frontmatter.
